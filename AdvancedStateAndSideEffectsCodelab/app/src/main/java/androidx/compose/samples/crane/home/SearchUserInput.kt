...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    ...
    val currentOnDestnationChanged by rememberUpdatedState(onToDestinationChanged)
    LaunchedEffect(key1 = editableUserInputState, block = {
        // snapshowFlow : State를 Flow로 변환한다.
        snapshotFlow { editableUserInputState.text }
            .filter { !editableUserInputState.isHint }
            .collect {
                currentOnDestnationChanged(editableUserInputState.text)
            }
    })
}
